...
const Timeline = () => {
  ...
  // ✅ No useEffect needed
  const interactions = getTimelineEvents();
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'call':
        return <img src="/src/assets/call.png" alt="call icon" className="w-8 h-8" />;
      case 'text':
        return <img src="/src/assets/text.png" alt="text icon" className="w-8 h-8" />;
      case 'video':
        return <img src="/src/assets/video.png" alt="video icon" className="w-8 h-8" />;
      case 'meetup':
        return '🤝';
      default:
        return '📌';
        ...
        <div className="space-y-4">
          {filteredInteractions.length > 0 ? (
            filteredInteractions.map((item) => {
              ...
              const displayName =
                item.friendName || friend?.name || 'Unknown';
            ...
            <div className="border border-dashed border-gray-200 rounded-2xl p-20 text-center">
              ...
              <p className="text-gray-400 text-sm mt-2">
                Go to a friend's page and tap Call, Text, or Video to log an interaction.
              </p>
            </div>
          ) : (
